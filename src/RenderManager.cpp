...
void RenderManager::Quit(){
    std::cout << "Model list clear test:" << std::endl;
    for(unsigned int i = 0; i < modelList.size(); i++){
        std::cout << "1" << std::endl;
        std::vector<ModelObject*>* modelObList = &models[modelList[i]];
        for(unsigned int t = 0; t < modelObList->size(); t++){
